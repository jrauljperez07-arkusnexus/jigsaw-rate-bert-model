from model import HateBertModel
...
@click.option('--optimizer', type=click.Choice(['Adam', 'AdamW', 'SGD'], case_sensitive=True), help='Adam/AdamW, SGD', default='Adam')
...
@click.option('--objective', type=click.Choice(['margin', 'bce'], case_sensitive=True), help='Margin loss (margin) or BCELoss(bce)', default='margin')
...
def main(
    ...
    model = HateBertModel(model_name=model_name)
    ...
    path = Path(data)
    df = pd.read_csv(path)
    train_loader = get_loader(df, tokenizer,  num_workers=num_workers, batch_size=batch_size, train=True)
